...
Business_centers = 'https://realt.by/newoffices/' # Базовый URL  - https://realt.by/sale/shops/
Malls = 'https://realt.by/malls/'
Offices = 'https://realt.by/sale/offices/'
Shops = 'https://realt.by/sale/shops/'
Restaurantes = 'https://realt.by/sale/restorant-cafe/'
Warehouses = 'https://realt.by/sale/warehouses/'
...
def del_space(string): # Для удаления спец пробела в ЦЕНЕ - 1 670. Возвращает целое число без пробела. На вход - строка с ценой '1 670' or '879'
    string = str(string)
    if ' ' in string:
    ...
    for i in range(1, 70):  # Видимо 70 - количество столбцов в екселе максимальное)))
        ...
        for field in list(project.keys()):  # проходимся по полям которые есть в конкретном project
            if field == ws.cell(row=3,
                                column=i).value and field == "ПДФ":  # и если поле в project совпадает с полем в ексель и это поле для гиперссылки  то записываем в строку
...
def add_projects_into_existing_excel(projects, excel_path = "Форма_04_Предложения_продажи_и_аренды.xlsx"):
    wb = openpyxl.load_workbook(filename=excel_path)
    # Выбираем лист
    ws = wb.get_sheet_by_name('Предложения')
...
def add_project_into_existing_excel(project, excel_path = "Форма_04_Предложения_продажи_и_аренды.xlsx"):
    wb = openpyxl.load_workbook(filename=excel_path)
    # Выбираем лист
    ws = wb.get_sheet_by_name('Предложения')
    row_num = ws.max_row  # находим последнюю строку (чтобы записывать новые данные в следующую)
...
def parse_object(obj_url, project={}):
    ...
    project['ПДФ'] = '{}/{}.html'.format("HTMLs", id_object_name)
    ...
    write_webpage_to_html(id_object_name, html_obj,
                          'HTMLs')  # write web page to html file
    ...
    for i in table:  # Проходимся по каждой строке на странице объявления
        ...
        option = i.find('td', {'class': "table-row-left"}).text
        if option in realt_fields_list:
            realt_answer = i.find('td', {'class': "table-row-right"}).text
            print(option)
            print(realt_answer)
            ...
            if option == "Площадь":
                ...
                print(Excel_field)
                print(Excel_field2)
                a = Excel_options_dict[Excel_field][realt_answer]
                b =  Excel_options_dict[Excel_field2][realt_answer]
                print(a, b)
                project[Excel_field] = Excel_options_dict[Excel_field][realt_answer]
...
# add_project_into_existing_excel(parse_object(baseurl), excel_path = "Форма_04_Предложения_продажи_и_аренды.xlsx")
'''
...
def parsing(event):
    ...
    root2 = Tk()
    root2.title("Parsing")
    l = Label(root2, text="Парсинг завершился!!! Проверяйте Ексель", font= "Arial 48", bg="green").pack()
...
btn.bind("<Button-1>", parsing)       #при нажатии ЛКМ на кнопку вызывается функция Hello

photo_button = PhotoImage(file='BIG-RED-BUTTON.png')
btn.config(image=photo_button, compound=CENTER)
l = Label(root, text="GO?", font= "Arial 48").pack()
btn.pack()
